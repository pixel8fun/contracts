...
contract ScriptBase is Script {
  ...
  function _getPuzzArtConfig(Config memory cfg) internal pure returns (PuzzArt.Config memory) {
    return PuzzArt.Config({
      owner: cfg.owner,
      minter: cfg.minter,
      devRoyaltyReceiver: cfg.devRoyaltyReceiver,
      devRoyaltyFeeBips: 500, /* 500 bips = 5% */
      defaultImage: DEFAULT_PUZZART_IMG,
  ...
  function _getLotteryNftConfig(Config memory cfg, address puzzArtAddress) internal pure returns (LotteryNFT.Config memory) {
    return LotteryNFT.Config({
      owner: cfg.owner,
      minter: puzzArtAddress,
  ...
  function _getMintSwapPoolConfig(Config memory cfg, address puzzArtAddress) internal pure returns (MintSwapPool.Config memory) {
